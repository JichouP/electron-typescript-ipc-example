...
const createWindow = (): void => {
  ...
  setTimeout(() => {
    ipcMain.send(mainWindow, 'showAlert', 'Hello World');
    ipcMain.send(mainWindow, 'printNumber', 100);
  }, 5000);
};
...
ipcMain.handle('repeatString', async (_, str) => {
  return str + str;
});
ipcMain.handle('twiceNumber', async (_, num) => {
  return num * 2;
});
